...
EDGE_FOLDER_NAME = "G3x3_edge_label"

IGNORE = 255

label_suffix='.png' # jpg for gan dataset, others : png

#从.txt文件中读取图像的名称，返回numpy.ndarray数据类型
...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        EDGE_path= get_edge_label_path(self.root_dir, self.img_name_list[index % self.A_size])
        
        edge_label = np.array(Image.open(EDGE_path), dtype=np.uint8)
        #  二分类中，前景标注为255
        ...
        # print(label.max())
        return {'name': name, 'A': img, 'B': img_B, 'L': label, 'E': edge_label}
