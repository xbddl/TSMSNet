...
class CDDataAugmentation:
    ...
    def transform(self, imgs, labels, edges, to_tensor=True):
        """
        :param imgs: [ndarray,]
        ...
        if len(edges) != 0:
            if edges[0].size != (self.img_size, self.img_size):
                edges = [TF.resize(img, [self.img_size, self.img_size], interpolation=0)
                    for img in edges]
        ...
        if self.with_scale_random_crop:
            ...
            edges = [pil_rescale(img, target_scale, order=0) for img in edges]
            # crop
            ...
            labels = [pil_crop(img, box, cropsize=self.img_size, default_value=255)
                    for img in labels]
            edges = [pil_crop(img, box, cropsize=self.img_size, default_value=0)
                    for img in edges]
        ...
        if to_tensor:
            ...
            labels = [torch.from_numpy(np.array(img, np.uint8)).unsqueeze(dim=0)
                      for img in labels]
            edges = [torch.from_numpy(np.array(img, np.uint8)).unsqueeze(dim=0)
                      for img in edges]
