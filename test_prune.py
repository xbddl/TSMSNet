...
def Test_prune(args):
    ...
    eval_dataloader = utils.get_loader(args.data_name, img_size=args.img_size,
                                  ...
                                  split='test')
    netprune = CDEvaluator(args, eval_dataloader)
    netprune.eval_models()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project_name', default='GVLM_Google_best', type=str)
    parser.add_argument('--checkpoint_root', default='prune', type=str)  #结果的保存路径
    ...
    parser.add_argument('--data_name', default='GVLM_Google', type=str) #数据集的路径,位于utils.py文件下的import data_config内部
