...
from models.loss2 import BCELoss
...
import pdb
from models.check_nan import check_is_all_nan
...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        loss_logger_path = os.path.join(args.checkpoint_dir, 'loss.txt')
        ...
        self.validate_precision = 1.0
        self.validate_recall = 1.0
    ...
    def _forward_pass(self, batch):
        ...
        self.pred_0, self.pred_1, self.pred_2, self.G_pred, self.gt, self.edge = check_is_all_nan(self.pred_0, self.pred_1, self.pred_2, self.G_pred, self.gt, self.edge)
        


    def _backward_G(self):
        
        self.G_loss = self._pxl_loss(self.pred_0, self.pred_1, self.pred_2, self.G_pred, self.gt, self.edge, self.validate_precision, self.validate_recall, self.epoch_id)
    ...
    def train_models(self):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            self._collect_epoch_states(False)
            ...
            for self.batch_id, batch in enumerate(self.dataloaders['val'], 0):
                with torch.no_grad():
                    self._forward_pass(batch)
                self._collect_running_batch_states()
            self._collect_epoch_states(True)
